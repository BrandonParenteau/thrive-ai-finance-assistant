...
function preprocessStatementText(text) {
    ...
    const firstDateIndex = transactionLines.findIndex(line => DATE_PATTERN.test(line.trim()) || BMO_DATE_PATTERN.test(line.trim()));
...
async function runGpt(openai, text, year) {
...
exports.parseStatement = (0, https_1.onRequest)({ timeoutSeconds: 300, memory: "1GiB", region: "us-central1", cors: false }, async (req, res) => {
    ...
    if (!checkRateLimit(`parseStatement:${uid}`, 5, 60 * 1000)) {
        res.status(429).json({ error: "Too many requests. Please wait before uploading another statement." });
        return;
    }
    const { text, pdfBase64 } = req.body;
    if (!text && !pdfBase64) {
        res.status(400).json({ error: "No statement content provided" });
        return;
    }
    ...
    if (pdfBase64 && typeof pdfBase64 === "string" && pdfBase64.length > MAX_PDF_B64) {
        res.status(413).json({ error: "PDF too large. Maximum size is 15MB." });
        return;
    }
    if (text && typeof text === "string" && text.length > MAX_TEXT) {
        res.status(413).json({ error: "Text too large. Maximum size is 500KB." });
        return;
    }
